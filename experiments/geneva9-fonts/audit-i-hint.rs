...
fn main(){
 ...
 for (target,ppem,name) in [(Target::Mono,8.,"mono8"),(Target::Mono,9.,"mono9"),(Target::Mono,10.,"mono10"),(Target::from(SmoothMode::Normal),8.,"gray8"),(Target::from(SmoothMode::Normal),9.,"gray9"),(Target::from(SmoothMode::Normal),10.,"gray10"),(Target::from(SmoothMode::Normal),36.,"gray36")]{
  let hint=HintingInstance::new(&outlines,Size::new(ppem),LocationRef::default(),target).unwrap();
  eprintln!("{name} hinting_enabled={}",hint.is_enabled());
  for code in [105u8] {
   let mut p=Path::default();outlines.get(cmap.map(code as char).unwrap()).unwrap().draw(DrawSettings::hinted(&hint,true),&mut p).unwrap();
   ...
   if name.starts_with("mono"){for m in &mut mask{*m=if *m>=128{255}else{0}}}
